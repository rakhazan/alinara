"use client";
import { MotionFeedback } from "@/components/ui/motion";
...
export default function CartPopup() {
  ...
  return <Popup open={open} onOpenChange={setOpen}>
    <PopupTrigger asChild><Button variant="ghost" size="icon" className="relative rounded-full" aria-label={`Keranjang, ${count} barang`}><HIcon icon={ShoppingBasket01Icon} className="size-5 lg:size-6" />{count > 0 && <MotionFeedback key={count} className="absolute -right-1 -top-1 inline-flex"><Badge className="px-1.5 text-[10px]">{count > 99 ? "99+" : count}</Badge></MotionFeedback>}</Button></PopupTrigger>
