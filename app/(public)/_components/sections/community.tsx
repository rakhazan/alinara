  ...
  return (
    <section aria-labelledby="community-heading" className="bg-surface px-4 pb-16 pt-6 lg:px-8 lg:pb-24 lg:pt-10">
      <div className="mx-auto max-w-[1920px]">
        <div data-scroll-reveal="rise" className="mb-8 text-center lg:mb-12">
        ...
        <ul className="grid grid-cols-2 gap-3 lg:grid-cols-4 lg:gap-6">
          {posts.map((post, index) => {
            ...
            return <li data-scroll-reveal="rise" data-scroll-delay={index * 0.04} key={post.id}>{post.href ? <Link href={post.href} className="block rounded-xl focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary">{content}</Link> : content}</li>;
          })}
        </ul>
      </div>
    </section>
