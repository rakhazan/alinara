      ...
      <div className="mt-8 grid items-start gap-8 lg:grid-cols-[320px_1fr]">
        <Card variant="muted" className="p-6">
          ...
          <div className="mt-6 space-y-3">{[5, 4, 3, 2, 1].map((star) => {
            ...
            return <div key={star} className="flex items-center gap-3 text-xs"><span className="text-secondary">{star} ★</span><Progress max={reviews.length || 1} value={count} aria-label={`${star} bintang: ${count} ulasan`} className="h-3 flex-1" /><span className="w-4 text-right">{count}</span></div>;
          })}</div>
        </Card>
        <div>
